...
class NaiveInitialInfectionParams(InitialInfectionParams):
    ...
    """
    def __init__(self, num_to_infect,per_to_Immune = 0.0,Immune_compliance = 1, city_name_to_infect=None,immune_source = InitialImmuneType.GENERAL_POPULATION,min_age = 0,people_per_day =0):
    ...
    def infect_simulation(self, sim, outdir):
        if self.immune_source == InitialImmuneType.GENERAL_POPULATION:
            sim.infect_random_set(self.num_to_infect, str(self),self.per_to_Immune,self.Immune_compliance, \
        ...
        if self.immune_source == InitialImmuneType.HOUSEHOLDS:
            sim.immune_households_infect_others(self.num_to_infect, str(self),self.per_to_Immune,self.Immune_compliance, \
             self.city_name_to_infect,self.min_age,people_per_day =self.people_per_day)
